...
async function checkPropiedadLimit(companiaId) {
    ...
    // Sin suscripción activa → bloquear con error especial
    ...
    const { count } = await supabase.from('propiedades')
        ...
        .is('deleted_at', null); // ← solo propiedades activas
...
async function checkTicketLimit(companiaId) {
    ...
    // FIX M-2: verificar suscripción activa (igual que checkPropiedadLimit)
    if (!c?.suscripcion_activa) {
        return { allowed: false, no_plan: true, plan: c?.plan || 'starter' };
    }
